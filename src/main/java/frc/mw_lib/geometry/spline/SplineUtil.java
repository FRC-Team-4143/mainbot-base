...
public class SplineUtil {
    ...
    public static ArrayList<Waypoint> subdividePoints(
        ...
        for (int i = 0; i < points.size() - 1; i++) {
            ...
            Translation3d p3 =
                    (i + 2 == points.size())
                            ? points.get(i + 1).translation
                            : points.get(i + 2).translation;

            Spline2d crs = new Spline2d(p0, p1, p2, p3);

            for (int j = 0; j < subdivisions.get(i); j++) {
                if (j == 0) {
                    subdividedPoints.add(
                            new Waypoint(crs.q(j * increments), points.get(i).required));
                } else {
    ...
    public static ArrayList<Waypoint> subdividePoints(
        ...
        for (int i = 0; i < points.size() - 1; i++) {
            subdivisions.add(
                    (int) Math.round(distance_between_points.get(i) * subdivisionsPerUnit));
        }
