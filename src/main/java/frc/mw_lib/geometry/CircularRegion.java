...
public class CircularRegion implements Region {
    ...
    /**
     * Log the circumference of the circle in 360 points. These can be visualized using
     * AdvantageScope to confirm that the regions are properly defined.
     */
    public void logPoints() {
        Translation2d[] points = new Translation2d[360];
        for (int i = 0; i < 360; i++) {
            points[i] =
                    center_.plus(new Translation2d(radius_, 0))
