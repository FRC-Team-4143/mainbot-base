    ...
    public void updateData(byte[] buffer) {
        ...
        if (timestamp.isLatest(timestamp_)) {
            ...
            if (piece_detection.detection_count_ != 0) {
                ...
                piece_detection.theta_x_ =
                        ByteBuffer.wrap(buffer, THETA_X, 8).getLong() / RESOLUTION;
                piece_detection.theta_y_ =
                        ByteBuffer.wrap(buffer, THETA_Y, 8).getLong() / RESOLUTION;
